...
const ServersPage: React.FC = () => {
  ...
  const [cloningServer, setCloningServer] = useState<Server | null>(null);
  const [viewingEndpointsServer, setViewingEndpointsServer] = useState<Server | null>(null);
  ...
  const handleViewEndpoints = (server: Server) => {
    setViewingEndpointsServer(server);
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-4 mb-8">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
          {t('pages.servers.title')}
        </h1>
        
        {/* Action buttons - responsive grid */}
        <div className="grid grid-cols-5 sm:flex gap-2 sm:gap-4">
          <button
            onClick={() => navigate('/market')}
            className="px-3 sm:px-4 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 rounded hover:bg-blue-200 dark:hover:bg-blue-800/40 flex items-center justify-center text-xs sm:text-sm btn-primary transition-all duration-200"
          >
          ...
          <button
            ...
            className={`px-3 sm:px-4 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 rounded hover:bg-blue-200 dark:hover:bg-blue-800/40 flex items-center justify-center text-xs sm:text-sm btn-primary transition-all duration-200 ${isRefreshing ? 'opacity-70 cursor-not-allowed' : ''}`}
          >
            {isRefreshing ? (
      ...
      {cloningServer && (
        <AddServerForm
          onAdd={handleCloneComplete}
          isOpen={true}
          onCancel={() => setCloningServer(null)}
          initialData={cloningServer}
        />
      )}
      ...
      {viewingEndpointsServer && (
        <EndpointsModal
          isOpen={true}
          onClose={() => setViewingEndpointsServer(null)}
          type="server"
          name={viewingEndpointsServer.name}
        />
      )}
