...
export const handleMcpPostRequest = async (req: Request, res: Response): Promise<void> => {  
  ...
  if (sessionId && transports[sessionId]) {  
    ...
    // Create a new transport for ANY request without a valid session ID  
    // This makes MCPHub more forgiving for clients that don't handle sessions properly  
    const tempSessionId = randomUUID();  
    transport = new StreamableHTTPServerTransport({  
      sessionIdGenerator: () => tempSessionId,  
      onsessioninitialized: (sessionId) => {  
        // Only store in transports if this is an initialize request  
        // For other requests, we handle them statelessly  
...
export const handleMcpOtherRequest = async (req: Request, res: Response) => {  
  ...
  let transport: StreamableHTTPServerTransport;  
  if (sessionId && transports[sessionId]) {  
    // Use existing transport if session ID is provided and valid  
    console.log(`Reusing existing transport for sessionId: ${sessionId}`);  
    transport = transports[sessionId].transport as StreamableHTTPServerTransport;  
  } else {  
    // Create a new stateless transport for requests without valid session ID  
    // This allows GET/DELETE requests to work without requiring session management  
    const tempSessionId = randomUUID();  
    transport = new StreamableHTTPServerTransport({  
      sessionIdGenerator: () => tempSessionId,  
      onsessioninitialized: (sessionId) => {  
        // Don't store stateless transports in the registry  
        console.log(`Created stateless transport: ${sessionId}`);  
      },  
    });  
  
    transport.onclose = () => {  
      console.log(`Stateless transport closed: ${transport.sessionId}`);  
      // No cleanup needed for stateless transports  
    };  
